...
Firstly, we propose two classification models using the mean embedding strategy, one is a multi-class classification model, and the other is a binary classification model. In the end, we discarded the multi-class classification model and kept the two better binary classification classifiers (we call them No. 1 and No. 2).

Then we proposed a binary classification model based on the GRU sequence model using the ```embeddings``` strategy and finally retained a better binary classifier based on the GRU sequence model (called that No. 3).
