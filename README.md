...
The training process is in the [mean/mean_trainer.ipynb](https://github.com/RepoMining/SimilarityCal/blob/main/mean/mean_trainer.ipynb) notebook.
...
However, the performance of No. 3 is close to No. 1 and No. 2 by using ```mean embedding strategy```, but No. 3 takes lots of training time. More details of training process can be found in the [sequential/sequentail_trainer.ipynb](baidu.com) file.
...
Finally, the similarity calculation result compared to RepoSim4Py is shown in the evaluation_comparison_result.csv file.
